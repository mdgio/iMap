            ...
            */
            ...
            , startup: function () {
                ...
                for (var j = 0; j < this.map.graphicsLayerIds.length; j++) {
                    ...
                    if (mapvariable.type == "Feature Layer" && mapServices.indexOf(mapvariable.url.replace("/" + mapvariable.layerId, "")) == -1) {
                        ...
                        layers.push({ id: mapvariable.id, name: mapvariable.name, url: mapvariable.url, type: mapvariable.type });
                    };
                }
                ...
                if (layers.length != 0) {
                    ...
                    $('<option />', { text: "Select a service", selected: true, style: "display:none" }).appendTo(this._layerCbo);
                    this._createCobos(this._layerCbo, layers, "name", "url");
                } else {
                    //if no available services to query on, show this to user
                    this._createCobos(this._layerCbo, [{ name: "You are unable to query this maps layers", url: "none"}], "name", "url");
            ...
            , _getServiceLayers: function (layerURL) {
                ...
                this._jsonGet(layerURL, lang.hitch(this, function (serviceJSON) {
                    ...
                    if (serviceJSON.layers) {
                        $('<option />', { text: "Select a layer", selected: true, style: "display:none" }).appendTo(this._subCbo);
                            ...
                            if (serviceJSON.layers[j].subLayerIds == null) {
                                this._jsonGet(layerURL + "/" + serviceJSON.layers[j].id, lang.hitch(this, function (lyrResp) {
                                    if (lyrResp.type == "Raster Layer") {
                                        $('<option disabled />', { text: lyrResp.name, value: lyrResp.id }).appendTo(this._subCbo);
                                    } else {
            ...
            , _getFields: function (layerURL) {
                ...
                this._jsonGet(layerURL, lang.hitch(this, function (layerJSON) {
                    ...
                    $('#fieldCbo').empty();
                    $('<option />', { text: "Select a field", selected: true, style: "display:none" }).appendTo(this._fieldCbo);
 ...
 */						
						...
						domConstruct.create('div', { id: "floatingPane" }, 'map');
							domConstruct.create('div', { id: "subContainer"}, "floatingPane");
								domConstruct.create('div', { id: "rowCountContainer", innerHTML: "Number of Features: <span class='displayRowCount'></span><br />"}, "subContainer");
								// This hidden area is needed for "output" before the CSV file could be downloaded
								var exContainer = domConstruct.create('div', { id: "exportInfoContainer", innerHTML: "<textarea type='hidden' id='output' style='display:none;'>stuff</textarea>"}, "subContainer"); 
								
						
						var exBtnAll = domConstruct.create('button', { id: 'exportButton', innerHTML: "Export All Rows to CSV" });
						...
						on(exBtnAll, "click", function(){
							dijit.byId("grid").exportGrid("csv", function(str){
								...
								*/
								...
								var HiddenForm = domConstruct.create('form', { id: "downloadform", innerHTML: "<method='post' action='' " + 
