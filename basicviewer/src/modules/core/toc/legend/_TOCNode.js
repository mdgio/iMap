...
define(["dojo/_base/declare", "dijit/_WidgetBase", "dijit/_TemplatedMixin", "dijit/_WidgetsInTemplateMixin"
    , "dojo/fx/Toggler", "dijit/form/HorizontalSlider", "dojo/dom", "dijit/registry", "dojo/query", "dojo/dom-class", "dojo/_base/lang", "esri/request", "dojo/topic"],
    function (declare, WidgetBase, TemplatedMixin, WidgetsInTemplateMixin, Toggler, Slider, dom, registry, query, domClass, lang, esriRequest, topic) {
        ...
        var _TOCNode = declare([WidgetBase, TemplatedMixin, WidgetsInTemplateMixin], {
            templateString: '<div class="agsjsTOCNode">' +
	            ...
	            '<span data-dojo-attach-point="labelNode" data-dojo-attach-event="onclick:_labelClick" style="cursor:pointer"></span>' +
	            '<span data-dojo-attach-point="dataLink"></span></span></div>' +
            ...
            _createLayerNode: function (layer) {
                ...
                if (layer._subLayerInfos) {// group layer
                    ...
                    if (this.rootLayer.url && this.rootLayer.url.indexOf("geodata.md.gov/imap/rest/services") > -1) {
                        if (this.layer && this.layer.id > -1) {
                            this._jsonGet("http://www.arcgis.com/sharing/rest/search/items/search?q=url:%22" + this.rootLayer.url + "%22 AND owner:mdimapdatacatalog&f=pjson").then(lang.hitch(this, function (json) {
                                if (json.results && json.results.length > 0 && json.results[0].id) {
                                    //var checkURL = esriRequest({ url: 'http://data.imap.maryland.gov/datasets/' + json.results[0].id + '_' + this.layer.id, handleAs: "xml" }).then(lang.hitch(this, function () {
                                    this.dataLink.innerHTML = "<a href='" + 'http://data.imap.maryland.gov/datasets/' + json.results[0].id + '_' + this.layer.id + "' target='_blank'><span class='imapDataLink'></span></a>";
                                    //}));
                                }
                            }));
                        } else if (!this.layer) {
                            if (parseInt(this.rootLayer.url.substr(this.rootLayer.url.length - 1)) != NaN && this.rootLayer.url.split("/")[this.rootLayer.url.split("/").length - 2] == "MapServer") {
                                var subUrl = this.rootLayer.url.substr(0, this.rootLayer.url.lastIndexOf("/"));
                                this._jsonGet("http://www.arcgis.com/sharing/rest/search/items/search?q=url:%22" + subUrl + "%22 AND owner:mdimapdatacatalog&f=pjson").then(lang.hitch(this, function (json) {
                                    if (json.results && json.results.length > 0 && json.results[0].id) {
                                        //var checkURL = esriRequest({ url: 'http://data.imap.maryland.gov/datasets/' + json.results[0].id + '_' + this.rootLayer.url.split("/")[this.rootLayer.url.split("/").length - 1], handleAs: "xml" }).then(lang.hitch(this, function () {
                                        this.dataLink.innerHTML = "<a href='" + 'http://data.imap.maryland.gov/datasets/' + json.results[0].id + '_' + this.rootLayer.url.split("/")[this.rootLayer.url.split("/").length - 1] + "' target='_blank'><span class='imapDataLink'></span></a>";
                                        //}));
                                    }
                                }));
                            }
